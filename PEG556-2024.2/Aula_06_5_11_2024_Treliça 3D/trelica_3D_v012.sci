...
fixed_dofs = [1:3, 4:6]; // Nós fixos (todos os graus de liberdade)
...
// Plot da estrutura original e deformada com marcação dos nós e grade ativada.
scf(1);
clf();
...
for i=1:n_elements
   ...
   if i==1 then 
       a=gca();
       a.tight_limits="on";
       a.data_bounds=[min(nodes(:,1))-0.5,min(nodes(:,2))-0.5;max(nodes(:,1))+0.5,max(nodes(:,2))+0.5];
       xlabel('X (m)');
       ylabel('Y (m)');
...
// Adicionando marcação dos nós.
for i=1:n_nodes 
   plot(nodes(i,1), nodes(i,2), 'ro', 'MarkerSize',10); 
end
...
for i=1:n_elements 
   ...
   plot(x,y,'r-','LineWidth',01);
end

// Impressão das tensões.
disp('Tensões nos elementos (MPa):');
for i=01:n_elements 
   printf('Elemento %d: %.02f MPa\n',i ,stresses(i)/1000000);
end

// Impressão dos deslocamentos máximos.
disp('Deslocamentos máximos:');
printf('X max: %.02f mm\n',max(abs(U(01:03:$)))*1000);
printf('Y max: %.02f mm\n',max(abs(U(02:03:$)))*1000);
printf('Z max: %.02f mm\n',max(abs(U(03:03:$)))*1000);
