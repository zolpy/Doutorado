...
// Criando figura e definindo proporções
f = gcf();
f.figure_size = [600, 600];

// Criando handles para legenda
h1 = [];
h2 = [];
...
for i = 1:size(elementos, 1)
    no1 = elementos(i,1);
    no2 = elementos(i,2);
    h1(i) = plot([xi(no1), xi(no2)], [yi(no1), yi(no2)], 'b-', 'LineWidth', 2);
end
...
for i = 1:size(elementos, 1)
    no1 = elementos(i,1);
    no2 = elementos(i,2);
    h2(i) = plot([xf(no1), xf(no2)], [yf(no1), yf(no2)], 'r--', 'LineWidth', 2);
end
...
// Configurando a legenda com os tipos corretos de linha
legends = ["Estrutura Original" "Estrutura Deformada (escala " + string(scale_factor) + "x)"];
e = gce();
e1 = e.children(1);  // Primeiro elemento plotado
e2 = e.children(2);  // Segundo elemento plotado
leg = legend([e1 e2], legends);
leg.position = "lower_right";
